# Instrucoes Operacionais para IA (Projeto Caixa de Amostras)
...
- Importar `dashboard_projeto2.json` e fazer `Deploy`.
...
Criterio de sucesso:
- gerar `Projeto2CaixaAmostras.elf` sem erro de compilacao/link.
...
- publicar comando:
  ...
  - payload: `ADMIN_SENHA`
- validar resposta fisica (display entra no modo admin e aceita troca de senha) e evento MQTT.

Checagem rapida para evitar confusao com firmware de outro projeto:
- Caixa de Amostras publica em `status`, `historico`, `sensores/temperatura`, `sensores/umidade` e `heartbeat`.
- Se aparecer apenas `sensores/luminosidade` e comando `IRRIGAR`, a placa pode estar com firmware da Estufa.
